...
public class QuickSort2Ways {
    ...
    private static int parttion(Comparable[] arr, int left, int right) {
        ...
        int j=right;
        while (true){
            while (arr[i].compareTo(v)<0&&i<=right){
                i++;
            }
            while (arr[j].compareTo(v)>0&&j>=left+1){
                j--;
            }
            if(i>j){
                break;
    ...
    public static void main(String[] args) {
        ...
        SortTestHelper.testSort("com.example.algorithm.sortadvance.quicksort.QuickSort2Ways", arr);
